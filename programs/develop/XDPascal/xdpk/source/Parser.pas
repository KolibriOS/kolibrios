...
case IdentKind of
  PROC, FUNC:
    ...
    if IdentPredefProc = EMPTYPROC then
      ...
      if IdentScope = GLOBAL then                                        // Assign an ordinal number used by the smart linker
        begin                                                            // (identical in both compilation passes)
        Inc(NumProcSeqs);
        if NumProcSeqs <= MAXPROCS then
          Ident[NumIdent].ProcSeq := NumProcSeqs;
        end;
      end
    else
      begin
...
procedure CompileBlock(BlockIdentIndex: Integer);
  ...
  procedure CompileProcFuncDeclarations(IsFunction: Boolean);
    ...
    if Tok.Kind = IDENTTOK then
      if Tok.Name = 'EXTERNAL' then      // External (Windows API) declaration  
        ...
        // Register import function (unless unreachable and discarded by the smart linker)
        if not IsSuppressedProc(NumIdent) then
    ...
    procedure CompileRoutineBody(ProcIdentIndex: Integer);
    var
      SavedCallerProcSeq: Integer;
      SavedCodeGenState: TCodeGenState;
      SavedInitializedGlobalDataSize, SavedUninitializedGlobalDataSize: Integer;
      Suppressed: Boolean;

    begin
    Suppressed := IsSuppressedProc(ProcIdentIndex);

    if Suppressed then
      begin
      SaveCodeGenState(SavedCodeGenState);
      SavedInitializedGlobalDataSize   := InitializedGlobalDataSize;
      SavedUninitializedGlobalDataSize := UninitializedGlobalDataSize;
      end;

    // Track the routine whose body is being compiled to attribute call graph edges to it
    SavedCallerProcSeq := CurCallerProcSeq;
    if Ident[ProcIdentIndex].ProcSeq > 0 then
      CurCallerProcSeq := Ident[ProcIdentIndex].ProcSeq;

    CompileBlock(ProcIdentIndex);

    CurCallerProcSeq := SavedCallerProcSeq;

    if Suppressed then
      begin
      // Unreachable routine: discard its code, relocations and static data (smart linking)
      RestoreCodeGenState(SavedCodeGenState);
      InitializedGlobalDataSize   := SavedInitializedGlobalDataSize;
      UninitializedGlobalDataSize := SavedUninitializedGlobalDataSize;
      end;
    end; // CompileRoutineBody
  ...
  if ForwardIdentIndex <> 0 then                                                    // Forward declaration resolution
    ...
    if not IsSuppressedProc(ForwardIdentIndex) then                    // The stub was not generated for a suppressed routine
      GenerateForwardResolution(Ident[ForwardIdentIndex].Address);

    CompileRoutineBody(ForwardIdentIndex);
    EatTok(SEMICOLONTOK);
