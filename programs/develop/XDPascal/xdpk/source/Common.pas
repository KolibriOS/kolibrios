...
const
  ...
  MAXPROCS                  = 2000;               // Maximum number of global routines tracked by the smart linker
...
type  
  ...
  TIdentifier = record
    ...
    ProcSeq: Integer;                           // Ordinal number of a global routine (in declaration order) for smart linking; 0 if not tracked
    IsUsed: Boolean;
  ...
  TProcGraphRow = array [0..MAXPROCS div 8] of Byte;
...
var
  ...
  // Smart linker state: the call graph between global routines is built during the first compilation pass;
  // during the second pass the code of unreachable routines is discarded
  ProcCallGraph: array [0..MAXPROCS] of TProcGraphRow;      // Row 0 holds references from the main program and unit-level code
  ProcIsLive: array [1..MAXPROCS] of Boolean;
  NumProcSeqs, CurCallerProcSeq: Integer;
  IsSecondPass, SuppressWarnings, DisableSmartLinking: Boolean;



procedure InitializeCommon;
procedure FinalizeCommon;
procedure InitializeSmartLinker;
procedure RecordCodeReference(Addr: LongInt);
procedure ComputeLiveProcs;
function IsSuppressedProc(IdentIndex: Integer): Boolean;
...
procedure InitializeSmartLinker;
begin
FillChar(ProcCallGraph, SizeOf(ProcCallGraph), #0);
FillChar(ProcIsLive, SizeOf(ProcIsLive), #0);

NumProcSeqs         := 0;
CurCallerProcSeq    := 0;
IsSecondPass        := FALSE;
SuppressWarnings    := FALSE;
DisableSmartLinking := FALSE;
end;




procedure AddCallGraphEdge(Caller, Callee: Integer);
begin
ProcCallGraph[Caller][Callee shr 3] := ProcCallGraph[Caller][Callee shr 3] or (1 shl (Callee and 7));
end;




function HasCallGraphEdge(Caller, Callee: Integer): Boolean;
begin
Result := ProcCallGraph[Caller][Callee shr 3] and (1 shl (Callee and 7)) <> 0;
end;
...
var
  IdentIndex: Integer;
begin
// Record a reference (call or address-taking) to a global routine at the given code address.
// Called from the code generator during the first pass to build the routine call graph
if IsSecondPass then Exit;

for IdentIndex := 1 to NumIdent do
  with Ident[IdentIndex] do
    if ((Kind = PROC) or (Kind = FUNC)) and (PredefProc = EMPTYPROC) and (ProcSeq > 0) and (Address = Addr) then
      begin
      AddCallGraphEdge(CurCallerProcSeq, ProcSeq);
      Exit;
      end;
end;
...
procedure ComputeLiveProcs;
var
  Queue: array [1..MAXPROCS] of Integer;
  QueueHead, QueueLen, NumTrackedProcs, Caller, Callee: Integer;
begin
// Smart linking disabled: keep all routines
if DisableSmartLinking then
  begin
  for Callee := 1 to MAXPROCS do ProcIsLive[Callee] := TRUE;
  Exit;
  end;

// Mark all routines reachable from the main program or unit-level code (caller 0)
NumTrackedProcs := NumProcSeqs;
if NumTrackedProcs > MAXPROCS then NumTrackedProcs := MAXPROCS;

QueueHead := 0;
QueueLen := 0;

for Callee := 1 to NumTrackedProcs do
  if HasCallGraphEdge(0, Callee) then
    begin
    ProcIsLive[Callee] := TRUE;
    Inc(QueueLen);
    Queue[QueueLen] := Callee;
    end;

while QueueHead < QueueLen do
  begin
  Inc(QueueHead);
  Caller := Queue[QueueHead];

  for Callee := 1 to NumTrackedProcs do
    if not ProcIsLive[Callee] and HasCallGraphEdge(Caller, Callee) then
      begin
      ProcIsLive[Callee] := TRUE;
      Inc(QueueLen);
      Queue[QueueLen] := Callee;
...
function IsSuppressedProc(IdentIndex: Integer): Boolean;
begin
// A routine is suppressed (its code is discarded) during the second pass if it is unreachable
Result := IsSecondPass and (Ident[IdentIndex].ProcSeq > 0) and not ProcIsLive[Ident[IdentIndex].ProcSeq];
end;
...
procedure Warning(const Msg: TString);
begin
if SuppressWarnings then Exit;    // Avoid duplicate warnings during the first smart linker pass
