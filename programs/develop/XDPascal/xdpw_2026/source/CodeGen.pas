...
type
  TCodeGenState = record
    CodeSize, NumRelocs: Integer;
  end;
...
procedure SaveCodeGenState(var State: TCodeGenState);
procedure RestoreCodeGenState(const State: TCodeGenState);
...
procedure SaveCodeGenState(var State: TCodeGenState);
begin
State.CodeSize := GetCodeSize;    // Also prevents the peephole optimizer from crossing the checkpoint
State.NumRelocs := NumRelocs;
end;
...
procedure RestoreCodeGenState(const State: TCodeGenState);
begin
// Discard all code and relocations generated since the checkpoint (used by the smart linker)
CodeSize := State.CodeSize;
NumRelocs := State.NumRelocs;
NumPrevCodeSizes := 0;            // Do not let the peephole optimizer touch discarded code
end;
...
procedure PushRelocConst(Value: LongInt; RelocType: TRelocType);
begin
if RelocType = CODERELOC then
  RecordCodeReference(Value);                            // The address of a routine is taken - the smart linker must keep it
...
if PopValueFromStack then
  ...
  begin
  if RelocType = CODERELOC then
    RecordCodeReference(Value);                                                 // A method address is taken - the smart linker must keep it
...
RecordCodeReference(EntryPoint);                 // Build the routine call graph for the smart linker
