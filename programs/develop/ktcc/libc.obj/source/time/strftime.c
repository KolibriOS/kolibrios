...
/*
 * KolibriOS: vendored from Arnold Robbins' public-domain strftime
 * (the "June 2015" revision, per the change log above). Public domain,
 * therefore compatible with this GPL-2.0 source tree.
 *
 * Local configuration:
 *   - Time-zone support is left disabled (HAVE_ANY_TM undefined): the %z
 *     and %Z specifiers are not compiled and fall through to the default
 *     handler, i.e. they are emitted literally ("%z"/"%Z").
 *   - %s (seconds since the Epoch) is disabled (HAVE_MKTIME undefined).
 * Keep changes to this file minimal to ease future syncs with upstream.
 */
...
strftime(char* s, size_t maxsize, const char* format, const struct tm* timeptr)
{
    ...
    /* KolibriOS: upstream used '>=', which wrongly failed when a %-less
       format plus its terminating NUL exactly fills the buffer. */
