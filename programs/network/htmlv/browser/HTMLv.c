	...
	loop()
	{
		...
		switch(EAX & 0xFF)
		{
			CASE evMouse:
				...
				IF (btn<>GetActiveProcess()) break; //если окно не активно на событи€ мыши не реагируем
				...
				IF (m.vert==65535) //прокрутка колЄсиком
				{
			...
			case evKey:
				...
				_EDIT_MARK:
				IF (key<>0x0d) && (key<>183) && (key<>184) && (key<>173) {EAX=key<<8; edit_box_key stdcall(#edit1);} //адресна€ строка
				break;
			case evReDraw:
				Draw_Window();
				break;
