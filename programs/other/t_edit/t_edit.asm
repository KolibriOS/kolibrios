;ќгромна€ благодарность Maxxxx32, Diamond, Heavyiron
;и другим программистам, а также
;“еплову јлексею (<Lrz> www.lrz.land.ru)


use32
  org 0x0
  db 'MENUET01' ;идентиф. исполн€емого файла всегда 8 байт
  dd 0x01
  dd start
...
;ѓаЃҐ•а™† ≠† б™ЃЂм™Ѓ г§†з≠Ѓ І†£гІ®Ђ†бм ≠†и† Ђ®°†
	mov	ebp,lib0
...
;---------------------------------------------------------------------
; читаем bmp файл с курсорами и лини€ми
...
mouse:
  ...
  test word [edit1.flags],10b;ed_focus ; •бЂ® ≠• Ґ дЃ™гб•, ҐлеЃ§®ђ
  jne still
...
;    jne still
  ...
  test word [edit2.flags],10b;ed_focus ; •бЂ® ≠• Ґ дЃ™гб•, ҐлеЃ§®ђ
  je @f
    cmp ah,0x80 ;if key up
    ja still
    cmp ah,42 ;[Shift] (left)
...
button:
;  cmp [menu_active],1 ;если нажали меню, то сначала реакци€ на меню
...
txtErrOpen db 'Н• ≠†©§•≠ д†©Ђ, ѓаЃҐ•амв• ѓа†Ґ®Ђм≠Ѓбвм ®ђ•≠®',0
txtErrIni0 db 'Н• Ѓв™алЂбп д†©Ђ б ®™Ѓ≠™†ђ®',0
err_ini0 db 0
...
if lang eq ru
  err_message_found_lib0 db 'ИІҐ®≠®в• ≠• г§†ЂЃбм ≠†©в® °®°Ђ®Ѓв•™г box_lib.obj',0
  head_f_i0:
  head_f_l0  db 'С®бв•ђ≠†п Ѓи®°™†',0
  err_message_import0 db 'Ои®°™† ѓа® ®ђѓЃав• °®°Ђ®Ѓв•™® box_lib.obj',0
  err_message_found_lib1 db 'ИІҐ®≠®в• ≠• г§†ЂЃбм ≠†©в® °®°Ђ®Ѓв•™г msgbox.obj',0
  err_message_import1 db 'Ои®°™† ѓа® ®ђѓЃав• °®°Ђ®Ѓв•™® msgbox.obj',0
else
