...
/* KolibriOS port: DOCX/ODT/OCR output writers depend on the 'extract' and
   tesseract thirdparty libs we do not ship. They are gated by #ifdef (not
   #if), so they can only be disabled by leaving the macros undefined here -
   command-line -D...=0 would still define them and pull in the writers. */
#if 0
